(function asteroidSetup() {
    ...
    function addAsteroid() {
        let asteroid = document.createElement('aside');
        ...
        let scale = (Math.random() * 1.3) + 0.3;
        let rotation = (Math.random() * 360) + 180;
        let skew = { x: Math.floor(Math.random() * 30), y: Math.floor(Math.random() * 30) };
        let startPos = Math.floor(Math.random() * 100);
        let endPos = Math.floor(Math.random() * 50) + ((startPos < 50) ? 50 : -50);
        ...
        let event = new CustomEvent('asteroidDetected', { detail: asteroid });
